...
import org.myutils.apis.CWithdrawAcc;
...
public class Wallet extends BaseTaggableObject
     ...
     * @throws MultipleOpReturnRequested if there is more than one OP_RETURN output for the resultant transaction.
     */
    public Transaction sendCoinsOffline(SendRequest request) throws InsufficientMoneyException,Exception {
        lock.lock();
        try {
            completeTx(request);
            ...
            // chaolumen start
            // 計算費用
            long fee = request.tx.getFee().getValue();
            List<TransactionOutput> outputs = request.tx.getOutputs();
            ...
            if (output2 != null){
                changeAddress = output2.getAddressFromP2PKHScript(request.getParams()).toString();
            }
            List<TransactionInput> inputs = request.tx.getInputs();
            StringBuffer sendAddress = new StringBuffer("");
            for(int i=0;i<inputs.size();i++){
                sendAddress.append(inputs.get(i).getFromAddress());
                if(i == inputs.size() -1){
                    continue;
                }
                sendAddress.append(",");
            ...
            try {
                ...
                BalanceLogDao balanceLogDao = session.getMapper(BalanceLogDao.class);
...
//                balanceLog.setSendAddress(sendAddress.toString());
     ...
     * @throws MultipleOpReturnRequested if there is more than one OP_RETURN output for the resultant transaction.
     */
    public SendResult sendCoins(TransactionBroadcaster broadcaster, Address to, Coin value) throws InsufficientMoneyException,Exception {
     ...
     * @throws MultipleOpReturnRequested if there is more than one OP_RETURN output for the resultant transaction.
     */
    public SendResult sendCoins(TransactionBroadcaster broadcaster, SendRequest request) throws InsufficientMoneyException,Exception {
     ...
     * @throws MultipleOpReturnRequested if there is more than one OP_RETURN output for the resultant transaction.
     */
    public SendResult sendCoins(SendRequest request) throws InsufficientMoneyException,Exception {
     ...
     * @throws MultipleOpReturnRequested if there is more than one OP_RETURN output for the resultant transaction.
     */
    public Transaction sendCoins(Peer peer, SendRequest request) throws InsufficientMoneyException,Exception {
        Transaction tx = sendCoinsOffline(request);
        peer.sendMessage(tx);
        return tx;
    }
